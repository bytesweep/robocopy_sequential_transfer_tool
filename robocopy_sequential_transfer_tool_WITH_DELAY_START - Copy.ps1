...
.DESCRIPTION
    ...
    Now includes configurable delayed start capability.
...
# Set delay in seconds (0 = no delay, 3600 = 1 hour, 46800 = 13 hours)
$DelaySeconds = 0

if ($DelaySeconds -gt 0) {
    $DelayHours = [math]::Floor($DelaySeconds / 3600)
    $DelayMinutes = [math]::Floor(($DelaySeconds % 3600) / 60)
    $DelaySecondsRemainder = $DelaySeconds % 60
    
    $DelayMessage = "Delaying start for "
    if ($DelayHours -gt 0) { $DelayMessage += "$DelayHours hour(s) " }
    if ($DelayMinutes -gt 0) { $DelayMessage += "$DelayMinutes minute(s) " }
    if ($DelaySecondsRemainder -gt 0) { $DelayMessage += "$DelaySecondsRemainder second(s)" }
    
    Write-Host $DelayMessage -ForegroundColor Yellow
    Start-Sleep -Seconds $DelaySeconds
}

# 3. Shared Parameters & Exclusions
